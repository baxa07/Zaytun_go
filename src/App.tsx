  ...
  return (
    <Shell>
      <main className="page">
        ...
        {featured.length>0&&<section className="bestseller-section" data-testid="bestseller-section"><div className="bestseller-heading"><div><p className="eyebrow">ZAYTUN TANLOVI</p><h2>🔥 Bestseller</h2><p>Oshxonamiz alohida tavsiya qiladigan taomlar</p></div></div>{featuredNotice&&<p className="bestseller-notice" role="status">{featuredNotice}</p>}<div className="bestseller-grid">{featured.map(item=><article className="bestseller-card" data-testid={`bestseller-${item.id}`} key={item.id}><Link to={`/menu/${item.id}`} className="bestseller-image"><ProductImage image={item.image} name={item.name}/><span className="bestseller-badge">🔥 Bestseller</span></Link><div className="bestseller-copy"><h3>{item.name}</h3>{item.description&&<p>{item.description}</p>}<div className="bestseller-footer"><b>{money(item.price)}</b>{item.modifiers?.length?<Link className="button bestseller-action" to={`/menu/${item.id}`}>Tanlash</Link>:<button className="button bestseller-action" type="button" onClick={()=>addFeatured(item)}>Savatga qo‘shish</button>}</div></div></article>)}</div></section>}
        <div className="chips">
          {categories.map((c) => (
            <button
