...
const ownerDraft=(item?:MenuItem):MenuItemDraft=>item?{
  ...
  packagingRequired:item.packagingRequired,packagingUnitPrice:item.packagingUnitPrice,packagingCapacity:item.packagingCapacity,
}:{categoryId:'',name:'',description:'',price:0,image:'',available:true,packagingRequired:true,packagingUnitPrice:3000,packagingCapacity:1};
...
function OwnerMenuManager(){
  ...
  return <Shell surface="staff"><main className="page owner-menu-page" data-testid="owner-menu-manager">
    ...
    <div className="owner-product-list">{filtered.map(item=>{const categoryName=categories.find(value=>value.id===item.categoryId)?.name||item.categoryId;return <article className="owner-product-card" key={item.id} data-testid={`owner-product-${item.id}`}><ProductImage image={item.image} name={item.name}/><div className="owner-product-main"><div><h3>{item.name}</h3><small>{categoryName}</small></div><b className="owner-price">{money(item.price)}</b><span className={item.available?'owner-status available':'owner-status'}>{item.available?'Sotuvda':'Sotuvda emas'}</span><small>{item.packagingRequired?`📦 ${money(item.packagingUnitPrice)} / ${item.packagingCapacity} porsiya yoki dona`:'Qadoqlashsiz'}</small></div><div className="owner-product-actions">{priceItem?.id===item.id?<div className="owner-quick-price"><input aria-label={`${item.name} yangi narxi`} inputMode="numeric" type="number" min="1" value={price} onChange={event=>setPrice(event.target.value)}/><button className="button primary" disabled={busyId===item.id} onClick={()=>void mutate(item,{price:Number(price)},'Narx yangilandi.').then(()=>setPriceItem(null)).catch(()=>undefined)}>Saqlash</button><button className="button text" onClick={()=>setPriceItem(null)}>Bekor</button></div>:<button className="button secondary" onClick={()=>{setPriceItem(item);setPrice(String(item.price))}}>Narx</button>}<button className="button secondary" disabled={busyId===item.id} onClick={()=>void mutate(item,{available:!item.available},item.available?'Mahsulot sotuvdan olindi.':'Mahsulot sotuvga qaytdi.').catch(()=>undefined)}>{item.available?'O‘chirish':'Sotuvga qo‘yish'}</button><button className="button text" onClick={()=>setEditor({mode:'edit',item})}>Tahrirlash</button></div></article>})}</div>
    <section className="owner-audit"><h2>So‘nggi o‘zgarishlar</h2>{audit.length===0?<p>Hali o‘zgarish yo‘q.</p>:audit.map(entry=><article key={entry.id}><b>{String(entry.afterState.name||entry.productId)}</b><span>{entry.action==='PRICE_CHANGED'?`Narx: ${money(Number(entry.beforeState?.price||0))} → ${money(Number(entry.afterState.price||0))}`:entry.action==='AVAILABILITY_CHANGED'?'Sotuv holati o‘zgardi':entry.action==='PACKAGING_CHANGED'?'Qadoqlash o‘zgardi':entry.action==='PRODUCT_CREATED'?'Mahsulot yaratildi':entry.beforeState?.image!==entry.afterState.image?'Rasm yangilandi':'Mahsulot yangilandi'}</span><small>{formatOperationalDateTime(entry.occurredAt)} · {entry.actorName||'Owner'}</small></article>)}</section>
  </main></Shell>
}
