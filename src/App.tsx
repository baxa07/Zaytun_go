  ...
  return (
    <Shell>
      <main className="page">
        ...
        {featured.length>0&&<section className="bestseller-section" data-testid="bestseller-section"><div className="bestseller-heading"><div><p className="eyebrow">ZAYTUN TANLOVI</p><h2>🔥 Eng ko‘p tanlanadigan</h2><p>Oshxonamiz alohida tavsiya qiladigan taomlar</p></div></div>{featuredNotice&&<p className="bestseller-notice" role="status">{featuredNotice}</p>}<div className="bestseller-grid">{featured.map(item=><article className="bestseller-card" data-testid={`bestseller-${item.id}`} key={item.id}><Link to={`/menu/${item.id}`} className="bestseller-image"><ProductImage image={item.image} name={item.name}/><span className="bestseller-badge">Ko‘p tanlanadi</span></Link><div className="bestseller-copy"><h3>{item.name}</h3>{item.description&&<p>{item.description}</p>}<div className="bestseller-footer"><b>{money(item.price)}</b>{item.modifiers?.length?<Link className="button bestseller-action" to={`/menu/${item.id}`}>Tanlash</Link>:<button className="button bestseller-action" type="button" onClick={()=>addFeatured(item)}>Savatga</button>}</div></div></article>)}</div></section>}
        <div className="chips">
          {categories.map((c) => (
            <button
...
function OwnerMenuManager(){
  ...
  const[search,setSearch]=useState('');const[category,setCategory]=useState('all');const[availability,setAvailability]=useState('all');const[packaging,setPackaging]=useState('all');
  ...
  return <Shell surface="staff"><main className="page owner-menu-page" data-testid="owner-menu-manager">
    ...
    <p className="owner-result-count">{filtered.length} ta mahsulot</p>
    <div className="owner-product-list">{filtered.map(item=>{const categoryName=categories.find(value=>value.id===item.categoryId)?.name||item.categoryId;return <article className="owner-product-card" key={item.id} data-testid={`owner-product-${item.id}`}><ProductImage image={item.image} name={item.name}/><div className="owner-product-main"><div><h3>{item.name}</h3><small>{categoryName}</small></div><b className="owner-price">{money(item.price)}</b><div className="owner-status-row"><span className={item.available?'owner-status available':'owner-status'}>{item.available?'Sotuvda':'Sotuvda emas'}</span>{item.isBestseller&&<span className="owner-status bestseller">🔥 Bestseller</span>}</div><small>{item.packagingRequired?`📦 ${money(item.packagingUnitPrice)} / ${item.packagingCapacity} porsiya yoki dona`:'Qadoqlashsiz'}</small></div><div className="owner-product-actions">{priceItem?.id===item.id?<div className="owner-quick-price"><input aria-label={`${item.name} yangi narxi`} inputMode="numeric" type="number" min="1" value={price} onChange={event=>setPrice(event.target.value)}/><button className="button primary" disabled={busyId===item.id} onClick={()=>void mutate(item,{price:Number(price)},'Narx yangilandi.').then(()=>setPriceItem(null)).catch(()=>undefined)}>Saqlash</button><button className="button text" onClick={()=>setPriceItem(null)}>Bekor</button></div>:<button className="button secondary" onClick={()=>{setPriceItem(item);setPrice(String(item.price))}}>Narx</button>}<button className={`button ${item.isBestseller?'bestseller-toggle active':'bestseller-toggle'}`} disabled={busyId===item.id} onClick={()=>void mutate(item,{isBestseller:!item.isBestseller},item.isBestseller?'Bestseller o‘chirildi.':'Bestseller yoqildi.').catch(()=>undefined)}>🔥 {item.isBestseller?'Bestseller OFF':'Bestseller ON'}</button><button className="button secondary" disabled={busyId===item.id} onClick={()=>void mutate(item,{available:!item.available},item.available?'Mahsulot sotuvdan olindi.':'Mahsulot sotuvga qaytdi.').catch(()=>undefined)}>{item.available?'O‘chirish':'Sotuvga qo‘yish'}</button><button className="button text" onClick={()=>setEditor({mode:'edit',item})}>Tahrirlash</button></div></article>})}</div>
