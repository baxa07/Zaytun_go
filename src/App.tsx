...
function OwnerProductForm({state,categories,onCancel,onSave}:{state:OwnerEditorState;categories:{id:string;name:string}[];onCancel:()=>void;onSave:(draft:MenuItemDraft)=>Promise<void>}){
  ...
  useEffect(()=>setDraft(ownerDraft(state.item)),[state]);
  ...
  return <section className="owner-editor" data-testid="owner-product-form">
    ...
    <div className="owner-form-grid">
      ...
      <Field label="Rasm HTTPS manzili" value={draft.image} placeholder="https://…" onChange={value=>update('image',value)}/>
    ...
    {error&&<p className="error" role="alert">{error}</p>}
    <div className="actions"><button type="button" className="button secondary" onClick={onCancel}>Bekor qilish</button><button type="button" className="button primary" disabled={busy} onClick={async()=>{setError('');if(!draft.name.trim()||!draft.categoryId||draft.price<=0){setError('Nomi, kategoriya va to‘g‘ri narxni kiriting.');return}if(draft.packagingRequired&&(draft.packagingUnitPrice<=0||!draft.packagingCapacity||draft.packagingCapacity<=0)){setError('Quti narxi va sig‘imini tekshiring.');return}setBusy(true);try{await onSave(draft)}catch(failure){setError(failure instanceof Error?failure.message:'Saqlanmadi')}finally{setBusy(false)}}}>{busy?'Saqlanmoqda…':'Saqlash'}</button></div>
  </section>
}
...
function OwnerMenuManager(){
  ...
  return <Shell surface="staff"><main className="page owner-menu-page" data-testid="owner-menu-manager">
    ...
    <section className="owner-audit"><h2>So‘nggi o‘zgarishlar</h2>{audit.length===0?<p>Hali o‘zgarish yo‘q.</p>:audit.map(entry=><article key={entry.id}><b>{String(entry.afterState.name||entry.productId)}</b><span>{entry.action==='PRICE_CHANGED'?`Narx: ${money(Number(entry.beforeState?.price||0))} → ${money(Number(entry.afterState.price||0))}`:entry.action==='AVAILABILITY_CHANGED'?'Sotuv holati o‘zgardi':entry.action==='PACKAGING_CHANGED'?'Qadoqlash o‘zgardi':entry.action==='PRODUCT_CREATED'?'Mahsulot yaratildi':'Mahsulot yangilandi'}</span><small>{formatOperationalDateTime(entry.occurredAt)} · {entry.actorName||'Owner'}</small></article>)}</section>
  </main></Shell>
}
