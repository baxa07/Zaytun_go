...
import { ProductImage, isHttpsImageUrl } from "./components/ProductImage";
...
function Menu() {
  ...
  useEffect(()=>{const rail=categoryRailRef.current;const button=Array.from(rail?.querySelectorAll<HTMLButtonElement>('[data-category-target]')||[]).find(candidate=>candidate.dataset.categoryTarget===active);if(!rail||!button)return;const left=button.offsetLeft-(rail.clientWidth-button.offsetWidth)/2;rail.scrollTo({left:Math.max(0,left),behavior:'smooth'})},[active]);
  const scrollToCategory=(categoryId:string)=>{selectedCategoryRef.current=categoryId;setActive(categoryId);categorySectionsRef.current.get(categoryId)?.scrollIntoView({behavior:'smooth',block:'start'})};
  return (
    <Shell>
      <main className="page">
...
type OwnerEditorState={mode:'create'|'edit';item?:MenuItem};
const ownerDraft=(item?:MenuItem):MenuItemDraft=>item?{
  categoryId:item.categoryId,name:item.name,description:item.description,price:item.price,image:item.image,available:item.available,
  isBestseller:Boolean(item.isBestseller),packagingRequired:item.packagingRequired,packagingUnitPrice:item.packagingUnitPrice,packagingCapacity:item.packagingCapacity,
}:{categoryId:'',name:'',description:'',price:0,image:'',available:true,isBestseller:false,packagingRequired:true,packagingUnitPrice:3000,packagingCapacity:1};

function OwnerProductForm({state,categories,onCancel,onSave}:{state:OwnerEditorState;categories:{id:string;name:string}[];onCancel:()=>void;onSave:(draft:MenuItemDraft)=>Promise<void>}){
  const{ownerUploadMenuImage,ownerRemoveMenuImage}=useApp();
  const[draft,setDraft]=useState<MenuItemDraft>(()=>ownerDraft(state.item));
  const[busy,setBusy]=useState(false);const[error,setError]=useState('');const[selectedImage,setSelectedImage]=useState<File|null>(null);const[preview,setPreview]=useState('');
  useEffect(()=>{setDraft(ownerDraft(state.item));setSelectedImage(null);setPreview('');setError('')},[state]);
  useEffect(()=>{if(!selectedImage){setPreview('');return}const objectUrl=URL.createObjectURL(selectedImage);setPreview(objectUrl);return()=>URL.revokeObjectURL(objectUrl)},[selectedImage]);
  const update=<K extends keyof MenuItemDraft,>(key:K,value:MenuItemDraft[K])=>setDraft(current=>({...current,[key]:value}));
  const chooseImage=(file:File|undefined)=>{if(!file)return;const validationError=validateMenuImageFile(file);if(validationError){setError(validationError);return}setError('');setSelectedImage(file)};
  return <section className="owner-editor" data-testid="owner-product-form">
    <div className="owner-editor-head"><div><p className="eyebrow">{state.mode==='create'?'YANGI MAHSULOT':'MAHSULOTNI TAHRIRLASH'}</p><h2>{state.item?.name||"Yangi taom qo‘shish"}</h2></div><button type="button" className="button text" onClick={onCancel}>Yopish</button></div>
    <div className="owner-form-grid">
      <Field label="Nomi *" value={draft.name} onChange={value=>update('name',value)}/>
      <label className="field"><span>Kategoriya *</span><select value={draft.categoryId} onChange={event=>update('categoryId',event.target.value)}><option value="">Tanlang</option>{categories.map(category=><option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
      <label className="field"><span>Narxi *</span><input inputMode="numeric" type="number" min="1" value={draft.price||''} onChange={event=>update('price',Number(event.target.value))}/></label>
      <Field label="Tavsifi" value={draft.description} onChange={value=>update('description',value)}/>
      <div className="owner-image-field" data-testid="owner-image-picker"><span className="field-label">Mahsulot rasmi</span>{preview?<img className="product-image-element" src={preview} alt={draft.name||'Tanlangan mahsulot rasmi'}/>:draft.image?<ProductImage image={draft.image} name={draft.name||'Mahsulot rasmi'}/>:<div className="owner-image-empty">Rasm tanlanmagan</div>}<div className="owner-image-actions"><label className="button secondary owner-file-button"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>{chooseImage(event.target.files?.[0]);event.currentTarget.value=''}}/>{preview||draft.image?'Rasmni almashtirish':'Rasm tanlash'}</label>{selectedImage&&<button type="button" className="button text" onClick={()=>setSelectedImage(null)}>Tanlovni bekor qilish</button>}{!selectedImage&&draft.image&&<button type="button" className="button text" onClick={()=>update('image','')}>Rasmni olib tashlash</button>}</div>{selectedImage&&<small>{selectedImage.name} · {(selectedImage.size/1024/1024).toFixed(1)} MB</small>}<details className="owner-image-advanced"><summary>HTTPS manzil (qo‘shimcha)</summary><Field label="Rasm HTTPS manzili" value={draft.image} placeholder="https://…" onChange={value=>update('image',value)}/></details></div>
      <label className="owner-check"><input type="checkbox" checked={draft.available} onChange={event=>update('available',event.target.checked)}/><span><b>Sotuvda</b><small>Mijoz menyusida ko‘rinadi</small></span></label>
      <label className="owner-check bestseller-owner-check"><input type="checkbox" checked={draft.isBestseller} onChange={event=>update('isBestseller',event.target.checked)}/><span><b>🔥 Bestseller</b><small>Zaytun tavsiyasi sifatida ajratiladi</small></span></label>
      <label className="owner-check"><input type="checkbox" checked={draft.packagingRequired} onChange={event=>{const enabled=event.target.checked;setDraft(current=>({...current,packagingRequired:enabled,packagingUnitPrice:enabled?(current.packagingUnitPrice||3000):0,packagingCapacity:enabled?(current.packagingCapacity||1):null}))}}/><span><b>Qadoqlash kerak</b><small>Alohida quti haqi olinadi</small></span></label>
      {draft.packagingRequired&&<><label className="field"><span>Quti narxi</span><input inputMode="numeric" type="number" min="1" value={draft.packagingUnitPrice||''} onChange={event=>update('packagingUnitPrice',Number(event.target.value))}/></label><label className="field"><span>Bir qutiga</span><input inputMode="numeric" type="number" min="1" value={draft.packagingCapacity||''} onChange={event=>update('packagingCapacity',Number(event.target.value))}/><small>porsiya yoki dona</small></label></>}
    </div>
    {error&&<p className="error" role="alert">{error}</p>}
    <div className="actions"><button type="button" className="button secondary" disabled={busy} onClick={onCancel}>Bekor qilish</button><button type="button" className="button primary" disabled={busy} onClick={async()=>{setError('');if(!draft.name.trim()||!draft.categoryId||draft.price<=0){setError('Nomi, kategoriya va to‘g‘ri narxni kiriting.');return}if(draft.packagingRequired&&(draft.packagingUnitPrice<=0||!draft.packagingCapacity||draft.packagingCapacity<=0)){setError('Quti narxi va sig‘imini tekshiring.');return}setBusy(true);let uploadedPath='';try{let nextDraft=draft;if(selectedImage){const uploaded=await ownerUploadMenuImage(selectedImage);uploadedPath=uploaded.path;nextDraft={...draft,image:uploaded.url}}await onSave(nextDraft)}catch(failure){if(uploadedPath)await ownerRemoveMenuImage(uploadedPath).catch(()=>undefined);setError(failure instanceof Error?failure.message:'Saqlanmadi')}finally{setBusy(false)}}}>{busy?(selectedImage?'Rasm yuklanmoqda…':'Saqlanmoqda…'):'Saqlash'}</button></div>
  </section>
}

function OwnerMenuManager(){
  const{categories,menuItems,refreshMenu,ownerUpdateMenuItem,ownerCreateMenuItem,ownerListMenuAudit}=useApp();
  const[search,setSearch]=useState('');const[category,setCategory]=useState('all');const[availability,setAvailability]=useState('all');const[packaging,setPackaging]=useState('all');const[photo,setPhoto]=useState('all');
  const[editor,setEditor]=useState<OwnerEditorState|null>(null);const[priceItem,setPriceItem]=useState<MenuItem|null>(null);const[price,setPrice]=useState('');const[busyId,setBusyId]=useState('');const[notice,setNotice]=useState('');const[error,setError]=useState('');const[audit,setAudit]=useState<MenuAuditEntry[]>([]);
  const reload=useCallback(async()=>{await refreshMenu();setAudit(await ownerListMenuAudit(12))},[ownerListMenuAudit,refreshMenu]);
  useEffect(()=>{void reload().catch(failure=>setError(failure instanceof Error?failure.message:'Menyu yuklanmadi'))},[reload]);
  const missingPhotoCount=useMemo(()=>menuItems.filter(item=>!isHttpsImageUrl(item.image.trim())).length,[menuItems]);
  const filtered=useMemo(()=>menuItems.filter(item=>item.name.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())&&(category==='all'||item.categoryId===category)&&(availability==='all'||(availability==='available'?item.available:!item.available))&&(packaging==='all'||(packaging==='required'?item.packagingRequired:!item.packagingRequired))&&(photo==='all'||(photo==='missing'?!isHttpsImageUrl(item.image.trim()):isHttpsImageUrl(item.image.trim())))),[availability,category,menuItems,packaging,photo,search]);
  const mutate=async(item:MenuItem,patch:Partial<MenuItemDraft>,message:string)=>{if(!item.updatedAt)throw new Error('Mahsulot versiyasi yuklanmadi. Yangilang.');setBusyId(item.id);setError('');try{await ownerUpdateMenuItem(item.id,item.updatedAt,patch);setNotice(message);setAudit(await ownerListMenuAudit(12))}catch(failure){setError(failure instanceof Error?failure.message:'Saqlanmadi');throw failure}finally{setBusyId('')}};
  return <Shell surface="staff"><main className="page owner-menu-page" data-testid="owner-menu-manager">
    <div className="page-title"><div><p className="eyebrow">OWNER</p><h1>Menu boshqaruvi</h1><p>Narxlar va sotuv holatini tez boshqaring.</p></div><button className="button primary" type="button" onClick={()=>setEditor({mode:'create'})}>+ Yangi taom</button></div>
    {notice&&<p className="success-notice" role="status">{notice}</p>}{error&&<p className="error" role="alert">{error}</p>}
    {editor&&<OwnerProductForm state={editor} categories={categories} onCancel={()=>setEditor(null)} onSave={async draft=>{if(editor.mode==='create'){await ownerCreateMenuItem(draft);setNotice('Yangi mahsulot qo‘shildi.')}else if(editor.item){await mutate(editor.item,draft,'Mahsulot yangilandi.')}setEditor(null);await reload()}}/>}
    <section className="owner-photo-progress" aria-label="Rasm holati"><div><span>Rasm kerak</span><strong>{missingPhotoCount}</strong><small>{menuItems.length} mahsulotdan</small></div><button type="button" className="button secondary" onClick={()=>setPhoto('missing')}>Rasmsizlarni ko‘rish</button></section>
    <section className="owner-filters"><Field label="Mahsulot qidirish" value={search} placeholder="Masalan, Jiz" onChange={setSearch}/><label className="field"><span>Kategoriya</span><select value={category} onChange={event=>setCategory(event.target.value)}><option value="all">Barchasi</option>{categories.map(value=><option key={value.id} value={value.id}>{value.name}</option>)}</select></label><label className="field"><span>Holati</span><select value={availability} onChange={event=>setAvailability(event.target.value)}><option value="all">Barchasi</option><option value="available">Sotuvda</option><option value="unavailable">Sotuvda emas</option></select></label><label className="field"><span>Qadoqlash</span><select value={packaging} onChange={event=>setPackaging(event.target.value)}><option value="all">Barchasi</option><option value="required">Qadoqlash kerak</option><option value="none">Qadoqlashsiz</option></select></label><label className="field"><span>Rasm</span><select value={photo} onChange={event=>setPhoto(event.target.value)}><option value="all">Barchasi</option><option value="missing">Rasmsiz</option><option value="ready">Rasmli</option></select></label></section>
    <p className="owner-result-count">{filtered.length} ta mahsulot{photo==='missing'?' · rasm qo‘shish kerak':''}</p>
    <div className="owner-product-list">{filtered.map(item=>{const categoryName=categories.find(value=>value.id===item.categoryId)?.name||item.categoryId;const missingPhoto=!isHttpsImageUrl(item.image.trim());return <article className={`owner-product-card${missingPhoto?' missing-photo':''}`} key={item.id} data-testid={`owner-product-${item.id}`}><ProductImage image={item.image} name={item.name}/><div className="owner-product-main"><div><h3>{item.name}</h3><small>{categoryName}</small></div><b className="owner-price">{money(item.price)}</b><div className="owner-status-row"><span className={item.available?'owner-status available':'owner-status'}>{item.available?'Sotuvda':'Sotuvda emas'}</span>{missingPhoto&&<span className="owner-status missing-photo">Rasm kerak</span>}{item.isBestseller&&<span className="owner-status bestseller">🔥 Bestseller</span>}</div><small>{item.packagingRequired?`📦 ${money(item.packagingUnitPrice)} / ${item.packagingCapacity} porsiya yoki dona`:'Qadoqlashsiz'}</small></div><div className="owner-product-actions">{priceItem?.id===item.id?<div className="owner-quick-price"><input aria-label={`${item.name} yangi narxi`} inputMode="numeric" type="number" min="1" value={price} onChange={event=>setPrice(event.target.value)}/><button className="button primary" disabled={busyId===item.id} onClick={()=>void mutate(item,{price:Number(price)},'Narx yangilandi.').then(()=>setPriceItem(null)).catch(()=>undefined)}>Saqlash</button><button className="button text" onClick={()=>setPriceItem(null)}>Bekor</button></div>:<button className="button secondary" onClick={()=>{setPriceItem(item);setPrice(String(item.price))}}>Narx</button>}{missingPhoto&&<button className="button owner-photo-action" onClick={()=>setEditor({mode:'edit',item})}>Rasm qo‘shish</button>}<button className={`button ${item.isBestseller?'bestseller-toggle active':'bestseller-toggle'}`} disabled={busyId===item.id} onClick={()=>void mutate(item,{isBestseller:!item.isBestseller},item.isBestseller?'Bestseller o‘chirildi.':'Bestseller yoqildi.').catch(()=>undefined)}>🔥 {item.isBestseller?'Bestseller OFF':'Bestseller ON'}</button><button className="button secondary" disabled={busyId===item.id} onClick={()=>void mutate(item,{available:!item.available},item.available?'Mahsulot sotuvdan olindi.':'Mahsulot sotuvga qaytdi.').catch(()=>undefined)}>{item.available?'O‘chirish':'Sotuvga qo‘yish'}</button><button className="button text" onClick={()=>setEditor({mode:'edit',item})}>Tahrirlash</button></div></article>})}</div>
    <section className="owner-audit"><h2>So‘nggi o‘zgarishlar</h2>{audit.length===0?<p>Hali o‘zgarish yo‘q.</p>:audit.map(entry=><article key={entry.id}><b>{String(entry.afterState.name||entry.productId)}</b><span>{entry.action==='PRICE_CHANGED'?`Narx: ${money(Number(entry.beforeState?.price||0))} → ${money(Number(entry.afterState.price||0))}`:entry.action==='AVAILABILITY_CHANGED'?'Sotuv holati o‘zgardi':entry.action==='BESTSELLER_CHANGED'?(entry.afterState.isBestseller?'Bestseller yoqildi':'Bestseller o‘chirildi'):entry.action==='PACKAGING_CHANGED'?'Qadoqlash o‘zgardi':entry.action==='PRODUCT_CREATED'?'Mahsulot yaratildi':entry.beforeState?.image!==entry.afterState.image?'Rasm yangilandi':'Mahsulot yangilandi'}</span><small>{formatOperationalDateTime(entry.occurredAt)} · {entry.actorName||'Owner'}</small></article>)}</section>
  </main></Shell>
}

function AuthGate({ children, surface }: { children: React.ReactNode; surface: "restaurant" | "driver" | "owner" }) {
  ...
  const [captchaKey, setCaptchaKey] = useState(0);
  const turnstileSiteKey = import.meta.env.VITE_TURNSTILE_SITE_KEY || "";
  const permitted = surface === "driver" ? role === "DRIVER" : surface==='owner'?role==='OWNER':role === "OWNER" || role === "RESTAURANT" || role === "DISPATCHER";
...
export default function App() {
  return (
    <><UpdateNotice/><Routes>
      ...
      <Route
        path="/owner/menu"
        element={<AuthGate surface="owner"><OwnerMenuManager /></AuthGate>}
      />
      <Route
        path="/restaurant"
        element={
