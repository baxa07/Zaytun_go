...
const mapOrder = (r: Row): Order => {
  ...
  return {
    ...
    items: ((r.order_items || []) as Row[]).map((i) => ({
      ...
      packagingRequired: Number(i.packaging_box_count) > 0,
      packagingBoxCount: Number(i.packaging_box_count || 0),
      packagingUnitPrice: Number(i.packaging_unit_price || 0),
      packagingCapacity: i.packaging_capacity == null ? null : Number(i.packaging_capacity),
      packagingTotal: Number(i.packaging_total || 0),
    ...
    packagingTotal: Number(r.packaging_total || 0),
...
export class SupabaseStore {
  ...
  private async submitOrderVia(rpcName: "create_public_order" | "create_customer_order", order: Order) {
    ...
    return {
      ...
      packagingTotal: data.packagingTotal,
