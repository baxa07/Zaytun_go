...
describe('Owner-curated Bestseller projection',()=>{
  ...
  it('keeps stable menu order and limits the premium section to four',()=>expect(featuredMenuItems(['a','b','c','d','e'].map(id=>item(id,{isBestseller:true}))).map(value=>value.id)).toEqual(['a','b','c','d']));
