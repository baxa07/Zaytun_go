...
test('switching pickup terminal to delivery resets payment to cash',async({page})=>{
  ...
  await page.getByLabel('Ism *').fill('Terminal Reset Mijoz')
  ...
  // Fulfillment type only lives on Step 1 -- back out to it to switch.
  ...
  await page.getByTestId('checkout-continue').click() // Step 1 -> Step 2 (map; delivery)
  await page.getByTestId('map-picker-set').click()
  await page.getByLabel('Kirish joyi xaritada to‘g‘ri belgilangan').check()
  await page.getByTestId('checkout-continue').click() // Step 2 -> Step 3 (address)
  ...
  await page.getByLabel('Kirish joyi xaritada to‘g‘ri belgilangan').check()
  await page.getByTestId('checkout-continue').click() // Step 3 -> Step 4 (payment)
...
test('empty basket cannot be submitted',async({page})=>{
  ...
  await page.getByLabel('Telefon *').fill('+998901112233')
  // Cart-emptiness is only checked by the final submit (Step 5) -- Step 1's
  // own Continue only validates contact fields, so an empty cart still
  // advances through the wizard and is caught at the real submit.
  await page.getByTestId('checkout-continue').click() // Step 1 -> Step 4 (payment; pickup skips 2/3)
