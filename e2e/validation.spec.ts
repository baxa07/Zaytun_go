...
test.describe('checkout validation', () => {
  test('rejects submission with an incomplete delivery address and names the missing fields', async ({page}) => {
    ...
    await expect(page.locator('.error')).toHaveCount(7) // name, phone, district, street, house, landmark/notes, coordinates
    ...
    await page.getByLabel('Mo‘ljal').fill('Maktab yonida')
    await page.getByTestId('checkout-submit').click()
    await expect(page).toHaveURL(/\/checkout$/)
    await expect(page.locator('.error')).toHaveText('Xaritadan joylashuvni belgilang')
