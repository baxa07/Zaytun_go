...
test('OWNER curates Bestseller and customer uses the premium card through the existing cart',async({browser})=>{
  ...
  await page.getByRole('button',{name:'Chiqish'}).click();await page.goto('/menu');const section=page.getByTestId('bestseller-section');await expect(section).toBeVisible();await expect(section.getByRole('heading',{name:'🔥 Eng ko‘p tanlanadigan'})).toBeVisible();const featured=page.getByTestId('bestseller-nacional-zhiz-file');await expect(featured).toBeVisible();await expect(featured.getByText('Ko‘p tanlanadi')).toBeVisible();await expect(featured.getByRole('button',{name:'Savatga'})).toBeVisible();expect(await section.locator('.bestseller-grid').evaluate(element=>getComputedStyle(element).gridTemplateColumns.split(' ').length)).toBe(2);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);
  await featured.getByRole('button',{name:'Savatga'}).click();await expect(page.getByTestId('cart-pill')).toContainText('1');await featured.getByRole('button',{name:'Savatga'}).click();await expect(page.getByTestId('cart-pill')).toContainText('2');
