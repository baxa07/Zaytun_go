...
test("P6 (real Supabase): a declined assignment is automatically and immediately reassigned to the other eligible driver, excluding the one who declined", async ({ browser }) => {
  const azizContext = await browser.newContext();
  const sardorContext = await browser.newContext();
  const customerContext = await browser.newContext();
  const staffContext = await browser.newContext();
  try {
    const aziz = await azizContext.newPage();
    const sardor = await sardorContext.newPage();
    ...
    // Test infra reliability: these 4 contexts were previously never
    // closed at all (pass or fail), leaking real browser processes and
    // open realtime websocket connections for the rest of the sequential
    // run -- a likely contributor to resource-contention flakiness in
    // later tests. try/finally guarantees cleanup even on assertion
    // failure, not just the happy path.
    await azizContext.close();
    await sardorContext.close();
